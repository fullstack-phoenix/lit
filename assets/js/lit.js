...
window.onload = () => {
   ...
   * Set active nav link
   */
  slice.call(document.querySelectorAll('.lit-nav a'), 0).forEach((field) => {
   ...
   * Flash Messages
   */
  slice.call(document.querySelectorAll('button.lit-flash-close'), 0).forEach((button) => {
    button.addEventListener('click', function () {
      const flashMessage = button.closest('.lit-flash')
   ...
   */
  const formFilters = document.querySelector('form#lit-filters-form')
  ...
  slice.call(document.querySelectorAll('.datepicker'), 0).forEach((field) => {
    new Pikaday({
      ...
      theme: 'lit-datepicker'
    })
  })
  /* eslint-enable no-new */

  slice.call(document.querySelectorAll('.lit-flash-close'), 0).forEach((field) => {
    field.addEventListener('click', function (e) {
      let el = field
      const selector = 'lit-flash'
